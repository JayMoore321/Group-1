...
<link href="navbar.css" rel="stylesheet" type="text/css" />  <!-- Link to the CSS pages -->


<link rel='shortcut icon' type='image/ico' href='favicon.ico' />  <!-- A favicon is added onto every page -->



<?php session_start();?>	<!-- The page session is started -->




<Title> Prism Games | Profile</title> <!-- Title for the page is created -->
...
<?php include('login-script.php'); ?> <!-- Use the login script-->
</div>
...
<?php 
$customerid =isset($_GET["customerid"]); <!-- Creating a variable -->
    include('DB_connection.php');  <!-- Use the script database connection -->

$sql= "select username,emailaddress,delivery_address,delivery_postcode,billing_address,billing_postcode from CUSTOMER WHERE username = '$_SESSION[$Session_name]'";  <!--Sql Query -->
$result=$db ->query($sql);
if(!$result){
die('There was an error running the query' .$db->error);  <!-- Error Checking -->
}

if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
$username=$row["username"];  <!-- Getting the data from the database -->
...
?>
<table width="400"   border="5" align="center" cellpadding="5" style="margin:30">
  <tr>
  ...
  <tr>
    ...
    <td width="165" valign="top"><?php echo ($username) ?></td>  <!-- Displaying the information from the database -->
  </tr>
  <tr>
    <td valign="top"><div align="left">Email Address:</div></td>
    <td valign="top"><?php echo ($emailaddress) ?></td>
  </tr>
  <tr>
    <td valign="top"><div align="left">Delivery Address:</div></td>
    <td valign="top"><?php echo ($delivery_address) ?></td>
  </tr>
  <tr>
    <td valign="top"><div align="left">Delivery Postcode:</div></td>
    <td valign="top"><?php echo ($delivery_postcode) ?></td>
  </tr>
  <tr>
    <td valign="top"><div align="left">Billing Address: </div></td>
    <td valign="top"><?php echo ($billing_address) ?></td

 </tr> 

 <tr>
    <td valign="top"><div align="left">Billing Postcode: </div></td>
    <td valign="top"><?php echo ($billing_postcode) ?></td
